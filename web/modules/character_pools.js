/**
 * character_pools.js — CharacterPoolStore singleton + fetch helper
 *
 * Schema: { version, pools: [{ id, label, tags: string[], notes, user }] }
 *
 * The built-in entry (id "default_animadex_1girl", user: false) is the default
 * character pool shipped in data/character_pool_default.json and served by the
 * backend. User pools (user: true) are editable / deletable and stored locally
 * in data/user_character_pools.json via the API.
 */

export const DEFAULT_POOL_ID = "default_animadex_1girl";

/**
 * Parse a comma/newline-separated pool string into a de-duplicated tag array
 * (case-insensitive). Mirrors python/character_pool.py::parse_pool.
 * @param {string} raw
 * @returns {string[]}
 */
 ...
 */
export async function fetchCharacterPools() {
  try {
    const resp = await fetch("/anima_prompt_helper/character_pools");
    if (!resp.ok) {
      console.warn("[AnimaPromptHelper] character_pools fetch failed:", resp.status);
      return null;
    }
    return await resp.json();
  } catch (err) {
    console.warn("[AnimaPromptHelper] character_pools fetch error:", err);
    return null;
  }
}
...
function init(data) {
  ...
  if (!data || !Array.isArray(data.pools)) {
    console.warn("[AnimaPromptHelper] CharacterPoolStore.init: invalid data");
    _notify();
    return;
  }
...
async function saveUserPool(pool) {
  try {
    const resp = await fetch("/anima_prompt_helper/user_character_pools", {
      method: "POST",
    ...
    console.warn("[AnimaPromptHelper] CharacterPoolStore.saveUserPool error:", err);
...
async function deleteUserPool(id) {
  ...
  try {
    const resp = await fetch(
      `/anima_prompt_helper/user_character_pools/${encodeURIComponent(id)}`,
      { method: "DELETE" }
    );
    if (!resp.ok) {
    ...
    console.warn("[AnimaPromptHelper] CharacterPoolStore.deleteUserPool error:", err);
    return { ok: false, error: "network_error" };
  }
}
...
export const CharacterPoolStore = {
