/**
 * character_randomizer_panel.js — DOM panel for AnimaCharacterRandomizer nodes.
 *
 ...
 *   - キャラを追加: a simple text input with Enter / 追加 button that appends a
 *     character tag to the active pool. Input `(`/`)` are backslash-escaped and
 *     `_` is normalized to spaces, matching danbooru tag formatting.
 ...
 *   - 対象 Composer dropdown + 「character欄へ挿入」 injects ``count`` random
 *     character tags straight into a same-graph AnimaPromptComposer's
 *     ``character`` field.
 *
...
/**
 * Normalize a raw character tag for insertion into the prompt pool:
...
function getActiveTags(node) {
  ...
  const builtin = CharacterPoolStore.getById(DEFAULT_POOL_ID);
...
/**
 * Build and attach the character-randomizer panel to a node.
...
export function injectCharacterRandomizerPanel(node) {
  ...
  headerTitle.textContent = "Anima Character Randomizer";
  ...
  // --- Add-character row (simple text input + button, no autocomplete) ---
  const addRow = document.createElement("div");
  addRow.className = "aph-ar-row aph-ar-add-row";
  const addInput = document.createElement("input");
  addInput.type = "text";
  addInput.className = "aph-ar-add-input";
  addInput.placeholder = "キャラタグを追加… (例: hatsune_miku)";
  ...
  function renderChips() {
    const tags = getPoolTags(node);
    summaryEl.textContent = `プール: ${tags.length} キャラ`;
    chipsEl.innerHTML = "";
  ...
  function rebuildPoolOptions() {
    ...
    const pools = CharacterPoolStore.getAll();
  ...
  function _refreshDelBtn() {
    const p = CharacterPoolStore.getById(poolSelect.value);
  ...
  // -------------------------------------------------------------------------
  // Add-character input handlers (no autocomplete; simple Enter / button)
  // -------------------------------------------------------------------------
  ...
  loadBtn.addEventListener("click", () => {
    ...
    renderChips();
    setStatus(`「${p.label}」を読み込みました (${(p.tags || []).length} キャラ)。`, false);
  });

  saveBtn.addEventListener("click", async () => {
    const tags = getPoolTags(node);
    if (!tags.length) { setStatus("プールが空です。先にキャラを追加してください。", true); return; }
    const selected = CharacterPoolStore.getById(poolSelect.value);
    const suggestedName = selected && selected.user ? selected.label : "My Characters";
    ...
    setStatus("保存中…", false);
    const res = await CharacterPoolStore.saveUserPool({ id, label: label.trim(), tags });
    if (res.ok) {
      rebuildPoolOptions();
      poolSelect.value = res.pool.id;
      _refreshDelBtn();
      setStatus(`「${res.pool.label}」を保存しました (${res.pool.tags.length} キャラ)。`, false);
    } else {
  ...
  delBtn.addEventListener("click", async () => {
    ...
    const res = await CharacterPoolStore.deleteUserPool(p.id);
    if (res.ok) {
      rebuildPoolOptions();
  ...
  insertBtn.addEventListener("click", () => {
    ...
    const slot = composer.inputs
      ? composer.inputs.find((i) => i.name === "character")
      : null;
    if (slot && slot.link != null) {
      setStatus("Composer の character は接続済みです。出力ポートを配線してください。", true);
      return;
    }
    const tags = getActiveTags(node);
    if (!tags.length) { setStatus("プールが空です。", true); return; }
  ...
  const widget = node.addDOMWidget(
    "anima_character_randomizer_panel", "div", panelEl, { serialize: false }
  );
  if (widget) {
  ...
  const unsubscribe = CharacterPoolStore.subscribe(rebuildPoolOptions);
