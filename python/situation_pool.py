"""situation_pool.py — pure helpers for the Anima situation randomizer.

Provides pool parsing, seeded (reproducible) selection, and lazy loading of
the built-in default pool (``data/situation_pool_default.json``). No I/O at
import time, so the selection logic is fully unit-testable without ComfyUI.
"""
...
# data/situation_pool_default.json — two levels up from python/
_DEFAULT_POOL_PATH = (
    Path(__file__).resolve().parent.parent / "data" / "situation_pool_default.json"
)
...
def load_default_pool() -> list[str]:
    """Return the built-in situation pool, cached for the process.

    Postconditions:
        - Returns a list of situation tags.
    ...
    try:
        ...
        logger.debug(
            "situation default pool loaded: %d tags from %s",
            len(_default_pool_cache),
            _DEFAULT_POOL_PATH,
        )
    except (FileNotFoundError, json.JSONDecodeError, OSError, ValueError) as exc:
        logger.warning(
            "situation_pool_default.json unavailable (%s); default pool empty", exc
        )
        _default_pool_cache = []
