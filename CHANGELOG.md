...
### Added
- **AnimaPromptComposer: three new optional STRING inputs `artist_extra` / `general_extra` / `natural_language_extra`.** They mirror the `lora_trigger_words` pattern — visible as multiline widgets on the node and accept wiring from upstream nodes (e.g. `AnimaCharacterRandomizer`'s `series` / `general` / `prompt_example`). At compose time the values are appended to the matching main widget with the canonical separator (`", "` for artist/general, `". "` for natural_language) BEFORE the standard tokenisation, so randomizer-supplied content flows into `positive_prompt` without overwriting whatever the user typed. `python/composer.py:join_fields` gains an `extras: dict[str, str] | None` parameter (recognised keys: `artist` / `general` / `natural_language`; unknown keys silently ignored; non-string values skipped). 17 new tests in `tests/test_composer.py` (52 total).
- **AnimaCharacterRandomizer: three new STRING outputs `series` / `general` / `prompt_example`.** Each picked character is looked up in `data/animadex_character_presets.json` and the matched presets' metadata is aggregated (series + general comma-joined with case-insensitive dedup; prompt_example newline-joined) so the same wiring that auto-fills Composer fields from a preset can now be driven by random picks. Hidden serialized widgets `picked_series` / `picked_general` / `picked_prompt_example` carry the GUI panel's resolved meta (which can also include user presets) into the workflow / image metadata; the headless / API path computes meta directly from the bundled animadex presets. `python/character_pool.py` gains `load_animadex_presets()` and `aggregate_meta(picks)`. Updated tests in `tests/test_character_randomizer.py` (12 new assertions, 33 total).
